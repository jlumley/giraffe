...
def test_get_payee(test_client):
    ...
    create_response = test_client.post("/payee/create", json=dict(name=uuid.uuid4().hex))
...
def test_update_payee(test_client):
    ...
    update_response = test_client.put(f"/payee/update/{payee_id}",json=dict(name=new_payee_name))
    payee_response = test_client.get(f"/payee/{payee_id}")
